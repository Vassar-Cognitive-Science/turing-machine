...
import { checkRuleTable, matchRule } from './machine'
import {
...
const isExpected = (sandbox, trial) => {
  ...
  const expectedTape = tape.strip(trial.expectedTape)
  const resultedTape = tape.strip(tape.tapeToArray(sandbox))
...
function createTrial (id,
  ...
  const pointer = (tapePointer !== undefined && tapePointer !== null) ? tapePointer : 0
...
function createSandbox (trial) {
  ...
  for (let i = 0; i < trial.startTape.length; i++) {
    const val = trial.startTape[i]
    sandbox = tape.appendAfterTail(sandbox, { val })
  }
...
export function deleteTrial (state, action) {
  ...
  const testReportId = new_state[action.id].testReportId
  if (new_state[testReportId]) { delete new_state[testReportId] }
...
export function addTrial (state, action) {
  ...
  const trial = createTrial(action.id,
    action.startState,
    action.startTape,
    action.expectedTape,
    action.tapePointer,
...
export function preRunTrial (state, action) {
  ...
  const testReportId = new_state[new_state[action.id].testReportId]
...
export function runTrial (state, action) {
  const trial = state[action.sourceId]
  ...
  while (sandbox.tapeInternalState !== HALT) {
    ...
    // match rule
    const ruleId = matchRule(state, sandbox.tapeInternalState, tape.read(sandbox))
    if (ruleId === null) {
      return reportTestResult(state, {
        sourceId: action.sourceId,
        name: trial.name,
  ...
  const result = isExpected(sandbox, trial)
...
export function reportTestResult (state, action) {
  ...
  const id = standardizeTestReportId(action.sourceId)
...
export function loadTrial (state, action) {
  const trial = state[action.id]
  const startTape = createTapeFromTrial(trial, true)
...
export function toggleIsRunningTrial (state, action) {
  ...
  const flag = (action.flag !== undefined) ? action.flag : !state.flag
...
export function clearTestResults (state, action) {
  ...
  for (let i = 0; i < new_state.testsById.length; i++) {
    ...
    const testReportId = new_state[new_state.testsById[i]].testReportId
...
export function toggleEditMode (state, action) {
  const isEdittingTrial = !state.isEdittingTrial
  ...
  if (isEdittingTrial) {
    ...
    const trial = new_state[action.id]
    ...
    const testReportId = new_state[new_state.edittingTrialId].testReportId
    // clear
    new_state.edittingTrialId = null
    new_state.edittingTrialName = null

    const originalTape = new_state.originalTape
...
export function saveTrialHelper (oldTrial, startTape, expectedTape, newName) {
  const trial = cloneTrial(oldTrial)
...
export function saveTrial (state, action) {
  ...
  const id = new_state.edittingTrialId
  const newName = new_state.edittingTrialName
  const oldTrial = new_state[id]
...
function loadTrialTapeHelper (state, sourceTape) {
  ...
  if (!tape.isTapeEmpty(sourceTape)) {
    const dummy = tape.standardizeCellId(sourceTape.tapeHead + 1)
...
export function createTapeFromTrial (trial, isTargetStartTape = true) {
  ...
  const sourceArray = (isTargetStartTape) ? trial.startTape : trial.expectedTape
  const sourceHead = (isTargetStartTape) ? trial.startTapeHead : trial.expectedTapeHead

  let offset = generatedTape.tapeHead - sourceHead
  if (offset === 0) {
    for (let i = 0; i < sourceArray.length; i++) {
      const val = tape.standardizeInputToTape(sourceArray[i], '')
