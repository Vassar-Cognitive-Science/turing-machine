...
export function findCell (state, id) {
  const cell = state[standardizeCellId(id)]
...
export function read (state) {
  const cur = findCell(state, state.tapePointer)
...
export function loadTape (state, tape) {
  const new_state = initializeTape(state, {})
...
export function lstrip (arr) {
  ...
  while (lmark < arr.length) {
    const val = standardizeReadFromTape(arr[lmark])
    if (val !== BLANK) { break }
    lmark++
  }
...
export function rstrip (arr) {
  ...
  while (rmark >= 0) {
    const val = standardizeReadFromTape(arr[rmark])
    if (val !== BLANK) { break }
    rmark--
  }
...
export function tapeToArray (tape) {
  const res = []
  if (isTapeEmpty(tape)) { return res }

  // run throught the linked list
  const dummy = standardizeCellId(tape.tapeHead + 1)
...
export function appendAfterTailHelper (state, val = null) {
  ...
  // find the oldTail
  const prev = findCell(state, state.tapeTail - 1)
  if (prev !== null) { prev.next = standardizeCellId(state.tapeTail) } // oldTail.next = new tail

  // create new tail
  const new_cell_id = standardizeCellId(state.tapeTail)
...
export function insertBeforeHeadHelper (state, val = null) {
  ...
  // find oldHead
  const next = findCell(state, state.tapeHead + 1)
  if (next !== null) { next.prev = standardizeCellId(state.tapeHead) } // oldHead.prev = new Head

  // create new Head
  const new_cell_id = standardizeCellId(state.tapeHead)
...
function expandBeforeHeadHelper (state, n = 1) {
  while (n--) { insertBeforeHeadHelper(state) }
}
...
export function writeIntoTapeHelper (state, pointer, val) {
  const target = findCell(state, pointer)
  const value = standardizeInputToTape(val, target.val)
...
export function fillTape (state, action) {
  ...
  if (action.id) {
    ...
    const position = action.position + state.anchorCell
...
export function moveTapeRightHelper (state, action) {
  ...
  const position = state.cellNum + state.anchorCell - 1
...
export function moveTapeLeftHelper (state, action) {
  ...
  const position = state.anchorCell
...
export function moveLeftHelper (state) {
  ...
  if (state.tapePointer < state.anchorCell) {
    ...
    if (state.anchorCell === state.tapeHead) { insertBeforeHeadHelper(state, null) }
  }
