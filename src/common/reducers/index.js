...
function rootReducer (state = initialState, action, clearRedo) {
  let s1, s2, s3, s4, s5
  // prepare undo/redo
  const cache = createEditHistoryCache(state, action)
  let new_state = state
  if (cache) {
...
function initializeMachine (state, action) {
  const new_state = gui.resizeScreenAndTape(initialState, { screenWidth: window.innerWidth })
...
function redo (state, action) {
  if (state.redoEditHistory.length === 0) { return state }

  const redoAction = state.redoEditHistory[state.redoEditHistory.length - 1]
...
function undo (state, action) {
  if (state.undoEditHistory.length === 0) { return state }

  const cache = state.undoEditHistory[state.undoEditHistory.length - 1]
  const undoAction = cache.undo
  let new_state
  ...
  const newRedoHistory = new_state.redoEditHistory.slice()
...
function guiReducer (state, action) {
  let new_state = state; let changed = true
  switch (action.type) {
    /* GUI info */
...
function ruleReducer (state, action, clearRedo) {
  ...
  switch (action.type) {
    ...
    case actionTypes.MOVE_TO:
      return table.moveTo(state, action)
      /* Rule actions */
    default:
      changed = false
  }
