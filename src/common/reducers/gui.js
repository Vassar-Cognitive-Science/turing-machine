...
export function adjustHeadWidthHelper (state, text) {
  ...
  if (textLength < defaultTextLength) {
    ...
    // else expand head
    const diff = textLength - defaultTextLength
...
export function resizeScreenAndTape (state, action) {
  const newScreenSize = action.screenWidth
  const newTapeSpace = newScreenSize * 0.9 - 96
  let newCellNum
  let new_state = state

  if (newTapeSpace < TAPE_BREAK_POINT) {
    const diff = Math.ceil((TAPE_BREAK_POINT - newTapeSpace) / 50)
  ...
  const originalPointer = new_state.tapePointer

  const midPoint = Math.floor(newCellNum / 2)
  ...
  while (offset--) {
    if (moveLeft) { new_state = moveTapeLeftHelper(new_state) } else { new_state = moveTapeRightHelper(new_state) }
  }
...
export function toggleTrialDrawer (state, action) {
  const flag = (action.flag !== undefined) ? action.flag : !state.trialDrawerToggle
  return Object.assign({}, state, {
...
export function toggleAnimation (state, action) {
  const flag = (action.flag !== undefined) ? action.flag : !state.animationOn
  return Object.assign({}, state, {
    animationOn: flag
  })
}
