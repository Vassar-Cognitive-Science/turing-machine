...
const focusOnPrev = () => {
  const prevId = getTapeCellNumber(document.activeElement.id) - 1
  document.getElementById(standardizeCellId(prevId)).focus()
}

const focusOnNext = () => {
  const nextId = getTapeCellNumber(document.activeElement.id) + 1
...
const mapStateToProps = (state, ownProps) => {
  const tar = state[standardizeCellId(state.anchorCell + ownProps.order)]
  const val = (tar !== undefined && tar !== null) ? tar.val : ''
