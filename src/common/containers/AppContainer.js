...
const handleAddTest = (dispatch, ownProps) => {
  dispatch(function (dispatch, getState) {
    ...
    // find suitable id
    while (getState().testsById.includes(id)) { id = standardizeTestId(TEST_ID++) }
...
const handleDeleteTests = (dispatch) => {
  dispatch(function (dispatch, getState) {
    const tests = getState().testsById
...
const handleRunAllTests = (dispatch, ownProps) => {
  ...
  dispatch(function (dispatch, getState) {
    for (let i = 0; i < getState().testsById.length; i++) { dispatch(preRunTrialAction(getState().testsById[i])) }
  })
...
const downloadAllTests = (dispatch, ownProps) => {
  dispatch((dispatch, getState) => {
    ...
    for (let i = 0; i < state.testsById.length; i++) {
      const id = state.testsById[i]
      const trial = Object.assign({}, state[id])
    ...
    const data = 'text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(suite, null, 4))
...
function onReaderLoad (event) {
  const trials = JSON.parse(event.target.result)
...
const mapStateToProps = (state, ownProps) => {
  ...
  for (let i = 0; i < state.testsById.length; i++) {
    const id = state.testsById[i]
    const obj = { id, name: state[id].name }
    idAndName.push(obj)
  }
